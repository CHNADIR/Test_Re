...
def count_occurrences_in_text(word, text):
    ...
    # transform text and word to lower case
    text = text.lower()
    word = word.lower()

    # if word consists of a single word
    if len(word.split()) == 1:
        # replace ponctuations in text with white spaces except for ' and -
        text = re.sub(r"[^\w\s'-]", ' ', text)

        array_text = text.split()

        # delete ' and _ if they figure in the beginning or ending of the text
        array_text[0], array_text[-1] = re.sub("[_'']", '', array_text[0]), re.sub("[_'']", '', array_text[-1])
               
        result = array_text.count(word)
    # if we are searching for a sentence
    else:
